  ...
  if (succeeded) {
    return (
      <div style={{ textAlign: "center", padding: "2rem" }}>
        ...
        <p style={{ color: "#627d98", fontSize: "0.85rem" }}>A receipt will be emailed to you shortly.</p>
      </div>
    );
  }
...
export default function PaymentForm({
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/customer/jobs/${jobId}/pay`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ amount, payment_type: paymentType, description }),
    })
      .then((r) => {
  ...
  }, [jobId, amount, paymentType, description]);
  ...
  return (
    <Elements stripe={stripeObj} options={{ clientSecret, appearance: { theme: "stripe", variables: { colorPrimary: "#f97316" } } }}>
