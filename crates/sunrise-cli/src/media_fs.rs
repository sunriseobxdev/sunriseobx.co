...
use proto::media_fs_client::MediaFsClient;
use proto::*;
...
impl GrpcMediaFs {
    ...
    fn client(&self) -> MediaFsClient<tonic::transport::Channel> {
        self.rt.block_on(async {
            MediaFsClient::connect(self.endpoint.clone()).await.expect("gRPC connect")
        })
    }
    ...
    fn grpc_readdir(&self, path: &str) -> Vec<CachedEntry> {
        {
            ...
            if let Some((ts, entries)) = cache.get(path) {
                if ts.elapsed() < Duration::from_secs(5) { return entries.clone(); }
            }
        }

        let entries: Vec<CachedEntry> = self.rt.block_on(async {
            let mut client = self.client();
            let req = self.auth_req(ReadDirRequest { path: path.to_string() });
            match client.read_dir(req).await {
                Ok(resp) => {
                    let prefix = if path.is_empty() { String::new() } else if path.ends_with('/') { path.to_string() } else { format!("{}/", path) };
                    resp.into_inner().entries.into_iter().map(|e| CachedEntry {
                        gcs_path: format!("{}{}{}", prefix, e.name, if e.is_dir { "/" } else { "" }),
        ...
        cache.insert(path.to_string(), (std::time::Instant::now(), entries.clone()));
        entries
    }
    ...
    fn grpc_stat(&self, path: &str) -> Option<(bool, u64)> {
        self.rt.block_on(async {
            ...
            client.stat(req).await.ok().map(|r| {
                let s = r.into_inner();
                (s.is_dir, s.size)
            })
        })
    ...
    fn grpc_read(&self, path: &str, offset: u64, size: u64) -> Vec<u8> {
        self.rt.block_on(async {
            let mut client = self.client();
            let req = self.auth_req(ReadRequest { path: path.to_string(), offset, size });
    ...
    fn grpc_delete(&self, path: &str) -> bool {
        self.rt.block_on(async {
            let mut client = self.client();
            client.delete(self.auth_req(DeleteRequest { path: path.to_string() })).await.is_ok()
        })
    }
    ...
    fn grpc_mkdir(&self, path: &str) -> bool {
        self.rt.block_on(async {
            let mut client = self.client();
            client.mkdir(self.auth_req(MkdirRequest { path: path.to_string() })).await.is_ok()
        })
    }
...
impl Filesystem for GrpcMediaFs {
    fn lookup(&mut self, _req: &Request<'_>, parent: u64, name: &OsStr, reply: ReplyEntry) {
        let parent_path = match self.inodes.lock().unwrap().get_path(parent) {
            Some(p) => p.to_string(), None => { reply.error(libc::ENOENT); return; }
        };
        let entries = self.grpc_readdir(&parent_path);
        let name_str = name.to_string_lossy();
        for e in &entries {
            if e.name == name_str.as_ref() {
                let ino = self.inodes.lock().unwrap().get_or_create(&e.gcs_path);
                reply.entry(&TTL, &self.make_attr(ino, e.is_dir, e.size), 0);
                return;
            }
        }
    ...
    fn getattr(&mut self, _req: &Request<'_>, ino: u64, _fh: Option<u64>, reply: ReplyAttr) {
        ...
        let path = match self.inodes.lock().unwrap().get_path(ino) {
            Some(p) => p.to_string(), None => { reply.error(libc::ENOENT); return; }
        };
        match self.grpc_stat(&path) {
            Some((is_dir, size)) => reply.attr(&TTL, &self.make_attr(ino, is_dir, size)),
    ...
    fn readdir(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, offset: i64, mut reply: ReplyDirectory) {
        ...
        for (idx, e) in entries.iter().enumerate() {
            let off = idx + 2;
            if off < i { continue; }
            let child_ino = self.inodes.lock().unwrap().get_or_create(&e.gcs_path);
            let ft = if e.is_dir { FileType::Directory } else { FileType::RegularFile };
            if reply.add(child_ino, (off + 1) as i64, ft, &e.name) { break; }
        }
        reply.ok();
    }

    fn read(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, offset: i64, size: u32, _f: i32, _lock: Option<u64>, reply: ReplyData) {
        let path = match self.inodes.lock().unwrap().get_path(ino) {
            Some(p) => p.to_string(), None => { reply.error(libc::ENOENT); return; }
        };
    ...
    fn write(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, _off: i64, data: &[u8], _wf: u32, _f: i32, _lock: Option<u64>, reply: ReplyWrite) {
        let path = match self.inodes.lock().unwrap().get_path(ino) {
            Some(p) => p.to_string(), None => { reply.error(libc::ENOENT); return; }
        };
        if self.grpc_write(&path, data) {
            if let Some(i) = path.rfind('/') { self.dir_cache.lock().unwrap().remove(&path[..i]); }
    ...
    fn unlink(&mut self, _req: &Request<'_>, parent: u64, name: &OsStr, reply: fuser::ReplyEmpty) {
        let pp = match self.inodes.lock().unwrap().get_path(parent) { Some(p) => p.to_string(), None => { reply.error(libc::ENOENT); return; } };
        let child = if pp.is_empty() { name.to_string_lossy().to_string() } else { format!("{}/{}", pp, name.to_string_lossy()) };
        if self.grpc_delete(&child) { self.dir_cache.lock().unwrap().remove(&pp); reply.ok(); }
        else { reply.error(libc::EIO); }
    }
...
#[cfg(unix)]
pub fn mount(grpc_endpoint: &str, token: &str, mountpoint: &str) -> Result<(), String> {
    // Verify connection
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let mut client = MediaFsClient::connect(grpc_endpoint.to_string()).await
            .map_err(|e| format!("Cannot connect to media server: {}", e))?;
    ...
    std::fs::create_dir_all(mountpoint).map_err(|e| format!("Cannot create mountpoint: {}", e))?;

    eprintln!("Mounted Sunrise Media at {}", mountpoint);
    eprintln!("  Server: {}", grpc_endpoint);
    eprintln!("  Ctrl+C to unmount");

    let fs = GrpcMediaFs::new(grpc_endpoint, token);
    fuser::mount2(fs, mountpoint, &[
        MountOption::FSName("sunriseobx".into()),
        MountOption::AutoUnmount,
    ]).map_err(|e| format!("FUSE: {}", e))
}

#[cfg(not(unix))]
pub fn mount(_endpoint: &str, _token: &str, _mountpoint: &str) -> Result<(), String> {
    Err("mount-media requires FUSE (Linux/macOS). On Windows, use `sunrise-cli media browse`.".into())
}
